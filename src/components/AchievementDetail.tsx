...
export function AchievementDetail({ achievement, onClose, onEdit, onDelete }: AchievementDetailProps) {
  ...
  return (
    <div className="modal-backdrop" onMouseDown={onClose} role="presentation">
      <article className="detail-drawer" onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-label={achievement.title}>
        ...
        <div className="detail-section">
          <span className="detail-label">مساهمتك</span>
          {achievement.actions.length ? <ol>{achievement.actions.map((action, index) => <li key={`${action}-${index}`}>{action}</li>)}</ol> : <p>لم تُسجل الإجراءات بعد.</p>}
        </div>
        ...
        <div className="detail-section">
          ...
          ) : <div className="missing-evidence">لا يوجد دليل بعد — أضف رابطًا، ملفًا أو رسالة Feedback.</div>}
        </div>
